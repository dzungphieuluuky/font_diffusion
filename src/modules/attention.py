...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, hidden_states, context=None):
        ...
        for block in self.transformer_blocks:
            ...
            )  # hidden_states: torch.Size([1, 4096, 128])
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, context=None):
        ...
        hidden_states = (
            self.attn1(self.norm1(hidden_states)) + hidden_states
        )  # hidden_states: torch.Size([1, 4096, 128])
        hidden_states = (
...
class OffsetRefStrucInter(nn.Module):
    def __init__(
        ...
        self.gnorm_s = torch.nn.GroupNorm(
            num_groups=num_groups,
            num_channels=style_feat_in_channels,
            eps=1e-6,
            affine=True,
        )
        ...
        self.content_proj_in = nn.Conv2d(
            res_in_channels, res_in_channels, kernel_size=1, stride=1, padding=0
        )
        self.gnorm_c = torch.nn.GroupNorm(
            num_groups=num_groups, num_channels=res_in_channels, eps=1e-6, affine=True
        )
        ...
        self.gnorm_out = torch.nn.GroupNorm(
            num_groups=num_groups,
            num_channels=style_feat_in_channels,
            eps=1e-6,
            affine=True,
        ...
        self.proj_out = nn.Conv2d(
            style_feat_in_channels, 1 * 2 * 3 * 3, kernel_size=1, stride=1, padding=0
        )
    ...
    def forward(self, res_hidden_states, style_content_hidden_states):
        ...
        style_content_hidden_states = style_content_hidden_states.permute(
            0, 2, 3, 1
        ).reshape(batch, height * width, s_channel)
        ...
        res_hidden_states = res_hidden_states.permute(0, 2, 3, 1).reshape(
