...
class SCR(nn.Module):
    def __init__(self, temperature, mode="training", image_size=96):
        super().__init__()
    ...
    def forward(self, sample_imgs, pos_imgs, neg_imgs, nce_layers="0,1,2,3,4,5"):
        ...
        sample_style_embeddings = self.StyleFeatProjector(
            self.StyleFeatExtractor(sample_imgs, nce_layers), nce_layers
        )  # out: N * C(2048)
        ...
        pos_style_embeddings = self.StyleFeatProjector(
            self.StyleFeatExtractor(pos_imgs, nce_layers), nce_layers
        )
        ...
        for i in range(num_neg):
            ...
            neg_style_embeddings_once = self.StyleFeatProjector(
                self.StyleFeatExtractor(neg_imgs_once, nce_layers), nce_layers
            )
            for j, layer_out in enumerate(neg_style_embeddings_once):
                if j == 0:
                    ...
                    neg_style_embeddings_mid = torch.cat(
                        [neg_style_embeddings_mid, layer_out[None, :, :]], dim=0
                    )
            if i == 0:
