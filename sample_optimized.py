...
def image_process(
    ...
    """
    char_to_use: Optional[str] = (
        content_character
        if content_character
        else getattr(args, "content_character", None)
    )
    ...
    with torch.no_grad():
        ...
        if getattr(args, "save_image", False):
            target_dir: str = os.path.join(
                args.save_image_dir, "TargetImage", font_name, style_name
            )
...
def sampling(
    ...
    with torch.no_grad():
        ...
        # Apply channels-last if enabled
        if getattr(args, "channels_last", False):
            content_image_tensor = content_image_tensor.to(
                memory_format=torch.channels_last
            )
            style_image_tensor = style_image_tensor.to(
                memory_format=torch.channels_last
            )
